...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		try{
			...
			var currency = $('#branch_currency').val();

			alert(currency);
			
			var val = code;
			var purchaserate =$('#name option').filter(function () 
				{ return this.value == val; }).data('purchaserate')*1; 
	...
	function addnewrow(){
		...
		var newTr = '<tr><th>' + rowCount +'</th><td ><input type="text" class="form-control productcode2" list="name" on autocomplete="off" name="productname" id="' +productcode +'"></td><td> <input type="number" value="1" class="form-control" autocomplete="off" id="' +hsn + '" name="productcode"></td><td><input type="text" id="' + size +
			'" class="form-control size" name="" value="0" ></td><td> <input type="number" value="1" class="form-control qty" autocomplete="off" id="' +qty + '" name="productcode"></td><td><input type="text" id="' + batch +
			...
			<div class="box-body">
				...
				<div class="row">
					<div class="col-md-4">
						<label>Voucher no :</label>
							<input type="text" name="voucherno" class="form-control" required autocomplete="off" id="voucherno" value="<?php echo $voucherno->result_array()[0]['NO'];?>">
											</div>
					<div class="col-md-4">
				...
				<div class="row">
					...
					<div class="col-md-4">
						...
						<input type="date" name="invoicedate" class="form-control" id="invoicedate" value="<?php $d=strtotime("now"); echo date("Y-m-d",$d);?>">
					</div>
					<div class="col-md-4">
						<input type="hidden" class="form-control" name="branch_currency" id="branch_currency">
					...
					<TABLE id="dataTable1" border="1" class="table table-striped table-hover">
						...
						<tbody id="dataTable" width="100%" border="1" class="table table-hover">
							...
							<?php foreach($product->result() as $gp){
                   			?>
							<option value="<?php echo $gp->pdt_name;?>" data-value ="<?php echo $gp->pdt_code;?>" data-tax="<?php echo $gp->tax;?>" data-taxlow="<?php echo $gp->taxlow;?>" data-taxhigh="<?php echo $gp->taxhigh;?>" data-slab="<?php echo $gp->slab;?>" data-purchaserate="<?php echo $gp->purchaserate;?>" data-hsncode="<?php echo $gp->hsncode;?>">
								<?php  } ?>
								<!-- <option value="HTML"> -->
						</datalist>
				...
				<div class="row margin" style="text-align: center">
					<div class="offset-md-4" style="display: inline-block">
						...
						<div style=" float: right;" class="col-md-4 alert-container">
...
$(document).ready(function() {
    $("#branch").change(function(event) {
        ...
        jQuery.ajax({
            type: "POST",
            url: "<?php echo base_url() ?>index.php/Onlinecontrol/FillCashBankLedgers",
            dataType: 'json',
            data: {id: id},
            success: function(data) {
                var bank = data['BankLedg'][0]['ledgerid'];
                var cash = data['CashLedg'][0]['ledgerid'];
                var currency = data['BranchCurrncy'][0]['currencyid'];


$('#cash option[value="'+cash+'"]').prop('selected', true);
$('#bank option[value="'+bank+'"]').prop('selected', true);
                $("#branch_currency").val(currency);


                // document.getElementById("cash").value = data['BankLedg'][0]['ledgername'];
            }
        });
    });
});
   ...
   if(loadinvno!=0){
	...
	$('#dataTable1').on('keyup change', '.qty', function() {
		try{ 
			...
			// $(this).closest("tr").find('.size').val(qty);
			var QTY = $('#qty' + a).val();
			var netsize = QTY;
			$('#size' + a).val('NA.'+netsize);
	...
	function rowcalculation(a) {
			...
			var purchaseratee = $('#unitprice' + a).val();

			var aed_rate = purchaseratee * 20;
			var purchaserate = aed_rate * 2/3;
...
$('#btnsave').on('click', function () {this.disabled=true; 
...
for (var i = 1; i < table.rows.length-1; i++) {
	no =document.getElementById("dataTable1").rows[i].cells[11].innerHTML;
	var product = $("#name option[value='" + $('#productcode'+no).val() + "']").attr('data-value');
...
{ 
	...
	$.ajax({
		type: "POST",
		url: "<?php echo base_url() ?>index.php/Purchasecontrol/inserttable_push",
		data: {
		...
		success: function (data) {
			...
			<button type="button" class="close" data- dismiss="alert" aria-hidden="true">×</button>Purchase Saved succesfully.. </div>`;
			$('.alert-container').html(alert); /* alert container class ".alert-container" */
			setTimeout(function(){
				location.reload();
			},3000);

			// alert("purchase saved succesfully");
			// // location.reload();
			clear();
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#btnupdate').on('click', function() {this.disabled=true;
					...
					success: function(data) {
						...
						$.ajax({
							...
							success: function(data) {
								...
								alert("purchase Updated succesfully");
								clear();
							}
...
n++;
	...
	mywindow.document.write('<tr><td colspan="9">Invoice value (in words): '+inwords+' Only</td></tr>');
