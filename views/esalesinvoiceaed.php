...
<script type="text/javascript">
  ...
  var totalstockamount =0;
	...
	$('#deliverycharge').val(deliverycharge);
			...
			var grandtotal=(netamount + taxamount - billdiscount - pointredeem + additionalcost+deliverycharge*1).toFixed(2);
				...
				<div class="row col-md-12">
					<div class="col-md-4">
						<div class="form-group">
							...
							<input type="text" class="form-control" name="deliverycharge" id="deliverycharge" value="20.00">
						</div>
						<div class="form-group">
				...
				<div class="row col-md-12 form-group margin txn-bottom-form" style="text-align: center;">
					...
					<div class="col-md-4">
						...
						<label for="child" style="margin-right: 20px; ">SHIPPING FROM</label>
							...
							<input type="radio" name="radioName" onclick="calc();" value="1" /> INR 
							<input type="radio"  checked="" name="radioName" value="3" /> UAE 
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog">
		...
		<div class="modal-content">
			...
			<button type="button" class="close" data-dismiss="modal">&times;</button>
                ...
                <textarea type="textarea" name="ship_address2" id="ship_address2" class="validate form-control">N/A</textarea>
				<span></span>
				</div>
				</div>
				...
				<div class="col-md-6">
					<div style="display: none;"  id="india" class="styled-input agile-styled-input-top">
			...
			<div style=""   id="emirates" class="styled-input agile-styled-input-top">
...
// }
				...
				<input type="text" class="form-control" value="United Arab Emirates" list="countrylist" required autocomplete="off"  name="country" id="country">
							<!-- <option value="0"  data-balance="0.00">Select Customer</option> -->
...
alert(err.message);
									...
									alert(myJSON);
                                           ...
                                           $.ajax({
												...
												data: {
													...
													'paidcash': paidcash*CONVERSIONRATE,
													'paidbank': paidbank*CONVERSIONRATE,
													'paymentmode': 'cash',
													'billdiscount':billdiscount*CONVERSIONRATE,
													'grandtotal':grandtotal*CONVERSIONRATE,
													'stock':totalstockamount,
													'cash': cash,
													'bank':bank,
													'customer':customer,
													'balance':balance*CONVERSIONRATE,
													'igst' :igst,
													'cgst' :cgst,
													'sgst' :sgst
												},
	...
	mywindow.document.write('<tr><td style="padding:5px;" colspan="7"><b>Billed to </b></td></tr>');

	mywindow.document.write('<tr><td style="padding:5px;" colspan="7">Name:'+customername+'<br>address:'+ship_address+'<br>State:'+state+'<br>Contact: '+number1+'</td></tr>');
    ...
    mywindow.document.write('<thead><tr><th style="padding:10px;">Particulars (Descriptions & Specifications)</th><th style="padding: 10px;">HSN / SAC Code</th><th style="padding: 10px;">Reference No</th><th style="padding: 10px;">Qty</th><th style="padding: 10px;">Rate</th><th style="padding: 10px;">Amount</th><th style="padding: 10px;">VAT</th></tr></thead><tbody>');
	...
	{  var no =document.getElementById("dataTable1").rows[i].cells[12].innerHTML;
		...
		// var productname = $("#name option[value='" + $('#productcode'+i).val() + "']").data('label');
			...
			mywindow.document.write('<tr><td style="padding: 20px;">'+productname+' (Size :'+sizename+', '+descri+')</td><td style="padding: 20px;">'+hsn+'</td><td style="padding: 20px;">'+refno+'</td><td style="padding: 20px;">'+qty+'</td><td style="padding: 20px;">'+mrp+'</td><td style="padding: 20px;">'+netamount+'</td>td style="padding: 20px;">'+igstt+'</td></tr>');
			n++;
	}
	...
	mywindow.document.write('<tr><td  colspan="3" rowspan="3">CASH PAYMENT :'+paidcash+'<br>BANK PAYMENT :'+paidbank+'<br>BALANCE TO PAY :'+balance+'</td><td colspan="4"><b>Sub Total 	                     - Rs: '+totalamount+'/-  </b></td></tr><tr><td colspan="4">VAT 	                   - Rs : '+igst+'/- </td></tr><tr><td colspan="4">Discount 	                   - Rs : '+billdiscount+'/- </td></tr><tr></tr>');

	mywindow.document.write('<tr><td colspan="3" style="font-size:22px;"><b>Grand Total</b></td><td style="font-size:30px;" colspan="4"><b>'+(grandtotal.toFixed(2))+'</b></td></tr>');

	// <tr><td colspan="5">Grand Total	 	                -  '+(grandtotal.toFixed(2))+'/- </td>
	mywindow.document.write('<tr><td colspan="7">Invoice value (in words): '+inwords+'</td></tr>');

	mywindow.document.write('<tr><td colspan="3">TERMS AND CONDITIONS:<br><br>Subject to ERNAKULAM Destribution<br><br><br><br>E & 0.E</td><td colspan="4">For MALL OF ABAYAS:<br><br>Authrised Signature<br><br>Certify that the particulars given above are true and correct<br></td></tr>');
